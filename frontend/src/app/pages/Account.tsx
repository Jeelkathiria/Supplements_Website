...
export const Account: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFB] overflow-hidden">
      {/* DESKTOP SIDEBAR */}
      <div className="hidden md:block flex-shrink-0">
        <div
          className={`h-screen bg-white border-r border-neutral-200 flex flex-col transition-all duration-500 ease-in-out ${
            isSidebarCollapsed ? 'w-20' : 'w-72'
          }`}
        >
          {/* Header with Logo & Toggle */}
          <div className={`flex items-center transition-all px-5 py-6 border-b border-teal-800 bg-gradient-to-r from-teal-700 to-teal-900 ${
          ...
          {/* Navigation Menu */}
          <nav className={`flex-1 space-y-1 transition-all ${
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center transition-all py-3 rounded-xl ${
                    isSidebarCollapsed 
                      ? 'justify-center px-0' 
          ...
          {/* Logout Button */}
          <div className={`border-t border-neutral-200 transition-all ${
            isSidebarCollapsed ? 'px-2 py-4' : 'px-3 py-4'
          }`}>
            <button
              onClick={handleLogout}
              className={`w-full flex items-center py-3 rounded-xl text-neutral-600 hover:bg-red-50 hover:text-red-600 transition-all ${
                isSidebarCollapsed ? 'justify-center px-0' : 'gap-3 px-3 justify-start'
              }`}
            >
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-64 bg-white border-r border-neutral-200 flex flex-col shadow-xl">
            {/* Header */}
            <div className="flex items-center gap-3 px-5 py-6 border-b border-neutral-100 justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-teal-900 flex items-center justify-center text-white font-bold">
                  S
                </div>
                <span className="font-bold text-neutral-900">Supplements</span>
              </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="md:hidden sticky top-0 z-30 bg-white border-b border-neutral-200 p-4 flex items-center justify-between">
